...
if __name__ == '__main__':
    ...
    print(len(pred_dict['pred_boxes']))
    print(pred_dict)
    print(pred_dict['pred_boxes'][0])
    print(pred_dict['pred_scores'][0])
    print(pred_dict['pred_labels'][0])
    ...
    for i in range(len(pred_dict['pred_boxes'])):
        ...
        axis_angles = np.array([0, 0, pred_dict['pred_boxes'][i][6] + 1e-10])
        rot = open3d.geometry.get_rotation_matrix_from_axis_angle(axis_angles)
        box3d = open3d.geometry.OrientedBoundingBox(center, rot, lwh)
        box3d.color = [1, 0, 0] # Set the color to red
